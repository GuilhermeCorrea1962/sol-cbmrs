...
@Component({
  ...
  template: `
    <div class="page-header">
      <div>
        <h1 class="page-title">Solicitacoes de Troca de RT</h1>
        <p class="page-subtitle">
          Solicitacoes de substituicao de Responsavel Tecnico aguardando aprovacao
        </p>
      </div>
    </div>

    <sol-loading [show]="loading()" message="Carregando solicitacoes de troca..." />
      ...
      @if (trocas().length === 0) {
        <mat-card appearance="outlined" class="empty-card">
          <mat-card-content>
            ...
            <p>Nenhuma solicitacao de troca pendente no momento.</p>
          </mat-card-content>
        </mat-card>
      } @else {
        <mat-card appearance="outlined">
          <mat-card-content class="table-container">
            <table mat-table [dataSource]="trocas()" class="full-width">
              ...
              <ng-container matColumnDef="acoes">
                ...
                <td mat-cell *matCellDef="let row">
                  <a mat-icon-button
                     [routerLink]="['/app/trocas', row.id]"
                     matTooltip="Analisar solicitacao">
...
export class TrocaFilaComponent implements OnInit {

  private readonly svc = inject(LicenciamentoService);

  readonly colunas = ['numero', 'tipo', 'status', 'municipio', 'area', 'entrada', 'acoes'];

  trocas  = signal<LicenciamentoDTO[]>([]);
  loading = signal(true);
  error   = signal<string | null>(null);

  ngOnInit(): void {
    this.svc.getFilaTrocaPendente().subscribe({
      next: page => {
        this.trocas.set(page.content);
        this.loading.set(false);
      },
      error: err => {
        this.error.set('Nao foi possivel carregar as solicitacoes de troca.');
        this.loading.set(false);
        console.error(err);
      }
    });
